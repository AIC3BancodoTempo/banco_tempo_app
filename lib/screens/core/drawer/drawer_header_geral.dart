...
class DrawerHeaderGeral extends StatelessWidget {
  ...
  UserModel _usuario (DocumentSnapshot snapshot){
    ...
    Stream<UserModel> get _user{
      return FirebaseFirestore.instance.collection("users").doc(userModel.key).snapshots().map(_usuario);
    }
        ...
        if (snapshot.hasData) {
          ...
          return UserAccountsDrawerHeader(
            ...
            currentAccountPicture: Stack(
              children: [
                GestureDetector(
                  onTap: (){Navigator.of(context).push(PageRouteBuilder(barrierDismissible: true,opaque: false,pageBuilder: (_, __, ___) => FullscreenImage(foto:usuario.foto)));},
                  child: CircleAvatar(
                    backgroundImage: usuario.foto != "" ? NetworkImage(usuario.foto) : AssetImage('assets/images/profile.png'),
                    radius: 90,
                  ),
                ),
                Positioned(
                  ...
                  child: IconButton(
                    onPressed: () async{ 
                      String foto = await add.pegarFoto(true, "Usuarios/${usuario.key}/foto_perfil",usuario.foto); 
      
                      await FirebaseFirestore.instance.collection("users").doc(usuario.key).update({"foto": foto});
                      
                      }, 
                    icon: Icon(Icons.camera_alt,color: white,size: 25,)
                  ),
                ),
                usuario.foto != "" 
                ?Positioned(
